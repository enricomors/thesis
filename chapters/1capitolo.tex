...
\end{itemize}L'algoritmo di consenso PoW è stata senza dubbio l'innovazione chiave. Con essa, Bitcoin risolve elegantemente quello che è stato uno dei problemi principali delle monete elettroniche fino a quel momento, il \textit{double spend}, ovvero quando la stessa unità di valuta può essere spesa in due o più transazioni differenti. L'invenzione di Nakamoto è anche una soluzione pratica al cosiddetto \textit{‘‘Problema dei Generali Bizantini’’}, ovvero il problema di accordarsi sul corso degli eventi, o sullo stato di un sistema, scambiando informazioni attraverso una rete non affidabile e potenzialmente compromessa. 
...
Oltre alle ricerce svolte per migliorare la scalabilità di Ethereum, che ha rilasciato una \textit{roadmap} che dovrebbe portare allo sharding, esiste un progetto interessante con l'obiettivo di pubblicare un nuovo registro distribuito che superi i limiti di scalabilità di blockchain come Bitcoin ed Ethereum.
